...
이번 배포에는 설정 화면, 다크 모드와 이메일 문의처럼 사용자 기능이 포함되어 있어 일반적인 semantic version 기준으로는 `2.3.0`도 후보가 될 수 있다.

그러나 Production `2.2.6`에 포함된 현재 인앱 업데이트 정책은 major 또는 minor가 상승하면 Immediate 강제 업데이트로 분류한다. 따라서 `2.3.0 (20300)`을 게시하면 기존 `2.2.6` 사용자가 새 버전을 선택 업데이트가 아닌 강제 업데이트로 받게 된다. 업데이트 정책은 사용자가 실행 중인 기존 앱에서 판정하므로 새 버전에만 수정 코드를 넣어서는 이 동작을 피할 수 없다.

이번 배포는 의도하지 않은 강제 업데이트를 피하기 위한 전환 버전으로 `2.2.10`을 사용한다. semantic version과 업데이트 긴급도를 `inAppUpdatePriority` 등으로 분리하는 작업은 #186에서 이어간다.
