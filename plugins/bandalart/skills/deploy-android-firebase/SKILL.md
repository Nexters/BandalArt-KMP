...
1. 배포를 시작하기 전에 `gh auth switch --hostname github.com --user easyhooon`으로 GitHub CLI의 활성 계정을 `easyhooon`으로 전환한다. `gh api user --jq .login`으로 활성 계정이 `easyhooon`인지 검증하고, 전환이나 검증에 실패하면 중단한다. 기존 `mps-jihun-lee` 계정으로 배포를 계속하지 않으며 인증 token은 출력하지 않는다.
2. 현재 branch와 `origin`의 동기화 상태를 확인한다. dirty/unpushed 변경이 있으면 배포 대상을 특정할 수 없으므로 중단한다.
3. Android Firebase App Distribution 설정을 확인한다.
...
4. credential은 존재 여부와 Git ignore 상태만 확인한다. 파일 내용, token, service account JSON과 webhook 값을 출력하지 않는다.
5. release notes가 없거나 비어 있으면 최근 Android 변경을 바탕으로 초안을 제안하고 사용자 확인 전에는 쓰거나 배포하지 않는다.
6. 아래 정보를 보여주고 실제 업로드 직전에 명시적 확인을 받는다.
   - Android build type
   - package name과 versionName/versionCode
   - tester group
   - release notes
   - 실행할 Gradle task 또는 저장소의 Android 업로드 명령
7. 확인 후 해당 APK build와 Firebase App Distribution 업로드를 실행한다. 배포 산출물 생성이 목적이므로 이 workflow에서는 Android build를 실행할 수 있다.
8. build나 upload가 실패하면 즉시 중단하고 secret을 가린 실패 단계와 원인을 보고한다.
9. 성공하면 Android APK, 앱 버전, tester group과 Firebase 배포 결과를 보고한다.
